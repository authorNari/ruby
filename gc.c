...
static size_t parallel_worker_threads(void);
...
#ifdef PARALLEL_GC_IS_POSSIBLE
    par_gc_threads_ptr = getenv("PARALLEL_GC_THREADS");
    initial_par_gc_threads = parallel_worker_threads();
    if (par_gc_threads_ptr != NULL) {
	int par_gc_threads_i = atoi(par_gc_threads_ptr);
	printf("parallel_gc_threads=%d (%d)\n", par_gc_threads_i,
